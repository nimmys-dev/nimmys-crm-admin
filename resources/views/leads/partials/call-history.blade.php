...
<x-card title="Call history">
    <x-slot:actions>
        <div class="flex items-center gap-3">
            ...
            @can('create', [App\Models\CallDetail::class, $lead])
                <button type="button" class="btn btn-primary btn-sm flex items-center gap-1.5 shadow-sm"
                    onclick="openModal('logCallModal')" style="white-space: nowrap;">
    ...
    <x-dashboard-table :rows="$callHistory" :headers="['Date', 'Time', 'Interest', 'Remarks', 'Next follow-up', 'Called by', ['label' => '', 'class' => 'w-px text-right']]" :empty-message="$callHasActiveFilters
            ? 'No calls match this search.'
            : 'No calls logged for this lead yet.'" empty-icon="ti ti-phone-off">
                        ...
                        @if ($call->is_item_sold === true)
                            <div>
                                ...
                                @if ($call->invoice_file_path)
                                    <a href="{{ Storage::url($call->invoice_file_path) }}" target="_blank"
        ...
        <div class="custom-modal-dialog custom-modal-dialog-lg">
            <div class="custom-modal-header">
                <div class="flex items-center gap-2.5">
                    <div
                        class="w-9 h-9 rounded-lg bg-primary/10 text-primary flex items-center justify-center font-bold text-lg">
                        <i class="ti ti-phone-outgoing"></i>
                    </div>
                    <div>
                        <h5 class="mb-0 text-base font-bold">Log Call Activity</h5>
                        <p class="text-xs text-muted mb-0">Record call outcome for <span
                                class="font-semibold text-body">{{ $lead->name }}</span> ({{ $lead->reference }})</p>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('leads.calls.store', $lead) }}" enctype="multipart/form-data"
                id="logCallForm">
                @csrf
                ...
                <div class="custom-modal-footer bg-light/30">
                    <button type="button" class="btn btn-secondary" onclick="closeModal('logCallModal')">
                        Cancel
                    </button>

                    <button type="submit" class="btn btn-primary flex items-center gap-1.5 shadow-sm">
...
@foreach ($callHistory as $call)
    @can('delete', $call)
        <x-delete-modal :id="'delete-call-' . $call->id" :action="route('leads.calls.destroy', [$lead, $call])"
            title="Delete call record" :message="'Remove the ' . $call->calledAt()->format('d-M-Y') . ' call from this history? The lead itself is kept.'" confirm="Remove call" />
    @endcan
@endforeach